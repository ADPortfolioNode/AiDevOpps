...
- Name: api.spec.ts >> Core API Workflows and URL Accessibility >> should handle API error for a bad URL during ingestion
- Location: api.spec.ts:86:3
...
> 93  |     expect(response.status()).toBe(400);
  ...
  181 |     } finally {
  182 |       if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
  183 |     }
  184 |   });
  185 | });
```